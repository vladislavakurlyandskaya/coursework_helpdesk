...
<div class="container">
    ...
    if (isset($_GET['take_id'])) {
        ...
        $check_application = mysqli_query($connect, "SELECT * FROM create_application WHERE id = {$_GET['take_id']}");
        if (mysqli_num_rows($check_application) == 1) {
            $application = mysqli_fetch_assoc($check_application);
    ...
    if ($result = mysqli_query($link, "SELECT * FROM create_application WHERE executor = (CONCAT_WS(' ', '$last_name','$first_name','$patronomic'))")) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<tr>' .
                ...
                "<td><a href='?take_id={$row['id']}'>Выполнить заявку</a></td>" .
